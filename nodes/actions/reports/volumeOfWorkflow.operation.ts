...
export const volumeOfWorkflowProperties: INodeProperties[] = [
	{
...
const displayOptions: IDisplayOptions = {
	show: {
		resource: ['reports'],
		operation: ['volumeOfWorkflow'],
	},
};

export const volumeOfWorkflowDescription = updateDisplayOptions(displayOptions, volumeOfWorkflowProperties);

export async function executeReportVolumeOfWorkflow(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
	...
	try {
		const response = await this.helpers.httpRequestWithAuthentication.call(this, 'wasapiApi', {
			method: 'GET',
			url: `${API_URL}/reports/volume-of-workflow?${queryParams.toString()}`,
