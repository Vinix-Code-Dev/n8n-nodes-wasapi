...
export const messagesMetricsProperties: INodeProperties[] = [...dateRangeProperties];

const displayOptions: IDisplayOptions = {
	show: {
		resource: ['dashboard'],
		operation: ['messagesMetrics'],
	},
};

export const messagesMetricsDescription = updateDisplayOptions(displayOptions, messagesMetricsProperties);

export async function executeDashboardMessagesMetrics(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
	...
	try {
		const response = await this.helpers.httpRequestWithAuthentication.call(this, 'wasapiApi', {
			method: 'GET',
			url: `${API_URL}/dashboard/metrics/messages?${queryString}`,
