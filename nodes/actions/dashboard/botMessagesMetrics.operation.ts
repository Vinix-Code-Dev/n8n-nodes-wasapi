...
export const botMessagesMetricsProperties: INodeProperties[] = [...dateRangeProperties];

const displayOptions: IDisplayOptions = {
	show: {
		resource: ['dashboard'],
		operation: ['botMessagesMetrics'],
	},
};

export const botMessagesMetricsDescription = updateDisplayOptions(displayOptions, botMessagesMetricsProperties);

export async function executeDashboardBotMessagesMetrics(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
	...
	try {
		const response = await this.helpers.httpRequestWithAuthentication.call(this, 'wasapiApi', {
			method: 'GET',
			url: `${API_URL}/dashboard/metrics/messages-bot?${queryString}`,
